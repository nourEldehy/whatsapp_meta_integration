...
class WhatsAppController(http.Controller):
    ...
    def webhook_receive(self, **kwargs):
        """
        Handles incoming message notifications from Meta.
        """
        data = json.loads(request.httprequest.data)
        _logger.info("Received WhatsApp Webhook Data: %s", json.dumps(data, indent=2))

        # Basic check to ensure it's a message payload
    ...
    def handle_text_message(self, message):
        """
        Processes a text message and posts it to the contact's chatter.
        """
        phone = message.get('from')
        body = message['text'].get('body')
        
        # Sanitize phone number if needed (e.g., remove '+')
        # This logic depends on how you store phone numbers in Odoo
        Partner = request.env['res.partner'].sudo()
        
        # Find partner by mobile or phone number
        partner = Partner.search(['|', ('phone', '=', phone), ('mobile', '=', phone)], limit=1)

        if not partner:
            # If no partner found, you can optionally create one
