...
class WhatsAppController(http.Controller):
    ...
    def whatsapp_webhook(self, **kwargs):
        ...
        try:
            ...
            # Find by last 9 digits
            search_fragment = normalized[-9:]
            lead = Lead.search(['|', ('phone', 'like', search_fragment), ('mobile', 'like', search_fragment)], limit=1)
            ...
            # Link partner if missing
            if not lead.partner_id:
                partner_match = Partner.search(['|', ('phone', 'like', search_fragment), ('mobile', 'like', search_fragment)], limit=1)
                if partner_match:
                    lead.partner_id = partner_match.id

            # Author: customer's partner or public partner
            try:
            ...
            # Post COMMENT with customer as author
            ctx = {
            ...
            # >>> NEW: open the 24h reply window
            lead.sudo().write({'last_whatsapp_reply_date': fields.Datetime.now()})

            # Explicit bus push (optional)
            try:
                formatted = new_msg.message_format()[0]
                dbname = request.env.cr.dbname
                Bus = request.env['bus.bus'].sudo()
                for pid in follower_partners:
                    Bus._sendone((dbname, 'res.partner', pid), {'type': 'mail.message/new', 'payload': formatted})
            ...
            # Force inbox notifications (optional)
            try:
                ...
                for pid in follower_partners:
                    Bus._sendone((dbname, 'res.partner', pid), {'type': 'mail.message/notification', 'payload': payload})
            except Exception as e:
